...
{
	int iRet = HttpRequestExec("GET", strUrl, NULL, strResponse);
//printf("%s\n",strResponse);
	//strResponse = strchr(strResponse,'{');
//printf("%s\n",strResponse);
	return iRet;
...
//if(fcntl(m_iSocketFd, F_SETFL, flags&~O_NONBLOCK) == -1) {//block miracle
		close(m_iSocketFd);
		m_iSocketFd = INVALID_SOCKET;
...
char* HttpRequest::HttpDataTransmit(char *strHttpHead, const int iSockFd)
{
	...
	while(1)
	{
		...
		//ret = recv(iSockFd, (void *)bufTemp, 4090,0); 
		if (ret == 0) //连接关闭
		{  		
			close(iSockFd);
			return NULL; 
